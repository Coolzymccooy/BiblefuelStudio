...
import { spaFallback } from "./src/lib/spaFallback.js";
...
// Media uploads now stream their RAW bytes straight to disk (see
// routes/media.js → receiveUploadToFile), so the JSON body parser no longer
// has to swallow ~270 MB base64 blobs. Cap JSON at a sane 25 MB: that's ample
// for the largest legitimate JSON payloads (transcript word arrays, render
// configs) while removing the old 300 MB in-memory liability that applied to
// EVERY endpoint. Raw octet-stream uploads bypass this parser untouched; a
// legacy base64 upload from a stale cached bundle is the only thing this can
// reject, and it self-heals on refresh.
...
// Fallback to serve the React app for client-side routing — see
// src/lib/spaFallback.js for why asset paths get an uncached 404.
